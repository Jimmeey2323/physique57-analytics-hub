...
export const InfoPopover: React.FC<InfoPopoverProps> = ({ context, locationId = 'all', className, size = 18, salesData, startOpen = false, startAsSidebar = true }) => {
  ...
  // Define custom styling for all info popover content
  const customInfoPopoverStyles = `
    ...
    if (isCompleteDocument) {
      ...
      const IframeRenderer = () => {
        ...
        React.useEffect(() => {
          ...
          // Write the complete HTML document with custom styles
          iframeDoc.open();
          // Inject custom styles into the document head if possible
          let modifiedHtml = html;
          if (html.includes('</head>')) {
            modifiedHtml = html.replace('</head>', `<style>${customInfoPopoverStyles.replace('.info-popover-content ', '')}</style></head>`);
          } else if (html.includes('<body>')) {
            modifiedHtml = html.replace('<body>', `<body><style>${customInfoPopoverStyles.replace('.info-popover-content ', '')}</style>`);
          }
  ...
  const safeHtml = DOMPurify.sanitize(contentWithoutStyles, DOMPURIFY_OPTS) as unknown as string;
    ...
    // Otherwise use custom styled wrapper with prose for better default typography
    return (
      <div className="info-popover-content">
        <style dangerouslySetInnerHTML={{ __html: customInfoPopoverStyles }} />
  ...
  const renderContentByMode = (nodes: React.ReactNode[]) => {
    ...
    return (
      <div className="info-popover-content space-y-2.5">
        <style dangerouslySetInnerHTML={{ __html: customInfoPopoverStyles }} />
        {nodes.map((node, idx) => (
          <div key={idx}>
