...
import { X, BarChart3, TrendingDown, Users, Calendar, MapPin, Clock } from 'lucide-react';
...
export const LateCancellationsDrillDownModal: React.FC<LateCancellationsDrillDownModalProps> = ({
  ...
  const safeRawData = Array.isArray(data?.rawData) ? data.rawData : [];
  const title = data?.title || 'Late Cancellations Detail';
  ...
  const metrics = useMemo(() => {
    ...
    const uniqueMembers = new Set(safeRawData.map((r: any) => r.memberId)).size;
    const uniqueLocations = new Set(safeRawData.map((r: any) => r.location)).size;
    const uniqueTrainers = new Set(safeRawData.map((r: any) => r.teacherName)).size;
    const uniqueClasses = new Set(safeRawData.map((r: any) => r.cleanedClass)).size;
    const totalRevenue = safeRawData.reduce((sum: number, r: any) => sum + (r.paidAmount || 0), 0);
    ...
    return {
      ...
      uniqueClasses,
      totalRevenue,
      avgPerMember: uniqueMembers > 0 ? (total / uniqueMembers).toFixed(2) : '0',
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="max-w-5xl max-h-[90vh] overflow-hidden flex flex-col">
        <DialogHeader className="bg-gradient-to-r from-red-600 to-orange-600 text-white px-6 py-4 -mx-6 -mt-6 mb-4">
          <div className="flex items-center justify-between">
            <DialogTitle className="flex items-center gap-3 text-white text-xl">
              <BarChart3 className="w-6 h-6" />
              <div>
                <p className="font-bold">{title}</p>
                <p className="text-red-100 text-sm font-normal">Detailed analysis of {formatNumber(metrics.total)} records</p>
              </div>
            </DialogTitle>
            <button
              onClick={onClose}
        ...
        <Tabs defaultValue="overview" className="flex-1 flex flex-col">
          <TabsList className="grid w-full grid-cols-3 bg-slate-100 mx-6 mb-6">
            <TabsTrigger value="overview" className="data-[state=active]:bg-red-600 data-[state=active]:text-white">Overview</TabsTrigger>
            <TabsTrigger value="distribution" className="data-[state=active]:bg-red-600 data-[state=active]:text-white">Distribution</TabsTrigger>
            <TabsTrigger value="records" className="data-[state=active]:bg-red-600 data-[state=active]:text-white">Records</TabsTrigger>
          </TabsList>
